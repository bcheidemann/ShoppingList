export type DropEventHandler<DragData, DropData> = (dragData: DragData, dropData: DropData) => void;
...
export class DragDropController<DragData, DropData> {
	...
	public onDragOverAllowDrop: svelte.JSX.DragEventHandler<EventTarget> = (e) => {
		e.preventDefault();
	};
	...
	public removeDropEventListener(handler: DropEventHandler<DragData, DropData>) {
		this.eventHandlers.drop = this.eventHandlers.drop.filter((h) => h !== handler);
	}
}
